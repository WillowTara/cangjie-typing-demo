...
### v2.0.1-hotfix (2026-03-09) - Lookup UI alignment and release hygiene
...
- ✅ 新增離線白名單污染字清理規則，避免 prompt 類內容再次混入（`src/features/typing/constants.ts`）
- ✅ 新增回歸測試：逐篇檢查離線白名單不含污染關鍵詞，並驗證原文長度與正規化後字數（`src/App.test.tsx`）
- ✅ 封版提交：`8760a55`
